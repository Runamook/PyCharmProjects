    ...
    def lock(self):
        ...
        while self.r.get(self.meter_number):
            ...
            if (now - start).total_seconds() > MeterBase.lock_limit:
                logger.error(f"Meter {self.meter_number} Unable to lock the meter in {MeterBase.lock_limit} seconds")
                raise IOError
...
class GetTime:
    ...
    def __init__(self, input_vars):
        ...
        self.meter_number = input_vars["meter"]["meterNumber"]
        self.results = dict()
        self.read_delta = 14        # Offset for reference time, cause value reading takes time
        self.write_delta = 15       # Offset for reference time, cause value writing takes time
    ...
    def _get(self, what):
        ...
        delta = datetime.timedelta(seconds=self.read_delta)
    ...
    def check_time(ref_value, checked_value):
        ...
        if checked_value != "error":
            ...
            now_date = "010101"
            # ValueError: time data '010000rror' does not match format '%y%m%d%H%M%S' - Fixed
            checked_value = "0000000"
    ...
    def set_time(self, cmd, data):
        with MeterBase(self.input_vars) as m:
            m.sendcmd_and_decode_response(b"/" + b"?" + self.input_vars["meter"]["meterNumber"].encode() + b"!\r\n")
            m.sendcmd_and_decode_response(MeterBase.ACK + b'051\r\n')
            result = m.sendcmd_and_decode_response(cmd.encode(), data.encode())
            cmd = MeterBase.SOH + b'B0' + MeterBase.ETX
            m.sendcmd_and_decode_response(cmd + MeterBase.bcc(cmd))
    ...
    def new_set(self, what):
        self.make_pause()
        dt = datetime.datetime.utcnow()
        if what == "date":
            value = dt.strftime("2" + "%y%m%d")
            obis = "0.9.2"
        elif what == "time":
            delta = datetime.timedelta(seconds=self.write_delta)
            value = (dt + delta).strftime("2" + "%H%M%S")
            obis = "0.9.1"
        else:
            logger.error(f"Meter {self.meter_number} unexpected key {what}, expecting 'date' or 'time'")
            raise KeyError

        logger.info(f"Setting {what} to {value}")
        logger.debug(f"===================== Setting {what} ===================== ")
        self.set_time(cmd="W5", data=f"{obis}({value})(00000000)")
...
def rq_create_p01_jobs(meter_list, test=False):
    ...
    """
    if test:
        p01_q = Queue(name="test-p01", connection=Redis(), default_timeout=3600)
    else:
        p01_q = Queue(name="p01", connection=Redis(), default_timeout=3600)
    logger.info("Connected to redis")
...
def rq_create_logbook_jobs(meter_list, test=False):
    if test:
        ...
        q = Queue(name=f"logbook", connection=Redis(), default_timeout=3600)
    logger.info("Connected to redis")
